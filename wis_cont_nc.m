function wis_cont_nc(track,modelnm,plotloc,bas,varargin)
%  map contour
p = inputParser;
p.addRequired('track');
...
for qq = 1:length(varname)
    data = read_wis_nc_var(varname{qq});
    %data.max = ncread('wis_pac_basin_l1_198101_max_mean.nc',[varname{qq},'_','max']);
    %data.mean = ncread('wis_pac_basin_l1_198101_max_mean.nc',[varname{qq},'_','mean']);
    for jtype = 1:2
